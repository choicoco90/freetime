...
data class FreeTimeGuestResponse(
    ...
    companion object {
        fun from(user: UserEntity, weeklyScheduleInfoResponse: WeeklyScheduleInfoResponse): FreeTimeGuestResponse {
            return FreeTimeGuestResponse(
                user = UserResponse.from(user),
                schedules = weeklyScheduleInfoResponse.schedules.map { DailyScheduleResponse.from(it.key, it.value) }
            )
        }
    }
}
